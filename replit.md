...
- **Webull Order History API**: The unofficial `webull` Python library (tedchou12/webull) had a hardcoded bug in `endpoints.py` where `startTime=1970-0-1` (malformed date format).
- **Resolution (Dec 23, 2025)**: Implemented a runtime monkey-patch in `selfbot_webull.py` that:
  1. Overrides the `_urls.orders()` method to remove the problematic `startTime` parameter
  2. Properly parses Webull's nested combo order response structure (orders are in `orders[]` array within each combo order)
- **Current Status**: Trade Monitor now successfully retrieves filled orders and posts them to Discord in the format `BTO AAPL 200C 12/26 @ 2.61 @everyone`
