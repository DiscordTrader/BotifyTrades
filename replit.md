...
The Order Management System (OMS) and Risk Management System (RMS) provide dynamic SL/PT management for signals that update via Discord message edits, with a WaxUI Entry Registry linking update signals to original entries. The Exit Order Arbiter arbitrates between signal-driven and risk-driven exit requests, enforcing that stop loss can never be lowered in hybrid mode. The Signal Exit Manager handles the complete order lifecycle with broker-aware modify flows. A Circuit Breaker provides emergency trading halt controls with global/per-channel halt, daily loss limit enforcement, and position count limits. Exit Strategy Modes include Signal Mode, Risk Mode, and Hybrid Mode.

Industry-grade Risk State Persistence ensures all risk state survives bot restarts:
- **Tier Hit State**: PT1-PT4 hit flags are persisted to database on every partial exit
- **Dynamic SL Price**: Current escalated stop loss price is saved after each profit target hit
- **Giveback Guard State**: Activation status and max P&L seen are persisted for continuity
- **Trailing Stop State**: Activation status and highest price are persisted for accurate trailing
- **Settings Versioning**: Risk settings hash stored per-trade enables prospective-only policy for mid-position settings changes (previously hit tiers remain hit, new thresholds apply going forward)
- **Startup Reconciliation**: On bot restart, full risk state is restored from database into position cache
...
- **BTO Signal Integration**: When BTO trades are executed with a routing_mapping_id, positions are automatically created in position_ledger for Signal Routing Live Positions tracking
...
- **Shared ExitArbiter**: Prevents duplicate STCs across signal and risk-driven exits with routing_mapping_id isolation for multi-mapping scenarios
- **Position State Gate**: Fresh position check after acquiring ExitArbiter lock prevents stale STC forwards when risk already closed the position (applied to both signal and risk flows)
- **Ledger-First Exit Ordering**: Updates position ledger BEFORE webhook delivery to prevent duplicates on retry
- **Race-Safe Exit Dedupe**: Unique constraint on (position_id, message_id) with IntegrityError handling prevents concurrent duplicate exits
