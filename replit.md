...
- **Sub-1s Order Execution Optimizations**: Cache-first position sizing (health monitor cache before live REST API for both options and stock paths), channel info DB cache with 10s TTL and invalidation on changes, bounded sync_ready gate (wait 5s then proceed instead of re-queue+2s sleep), signal price fallback always passed to all brokers to skip REST quote fetches, and granular [TIMING] markers throughout the worker and execution pipeline.
