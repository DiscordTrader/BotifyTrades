...
- **Phantom Position Detection**: Risk engine detects when broker reports "No stock/option position" on STC attempts. Uses `no_position_streak` counter: after 3 consecutive "no position" failures, position is marked as permanent failure and removed from risk tracking. Streak resets if a different error type occurs (e.g., transient broker error), preventing false positives on real positions with intermittent API issues. Prevents infinite retry loops for phantom positions (positions in hub cache that never actually filled).
