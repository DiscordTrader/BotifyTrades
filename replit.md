...
- **Conditional Order Execute Flag Guard**: All conditional order creation paths (ProTrader/format registry, India conditional, Hengy alerts, general COND ORDER) now check `execute_enabled` before creating orders. Previously, conditional orders bypassed the channel's execute flag — signals from channels with `execute=0` (track-only mode) would still create and trigger real broker orders. The fix adds an `execute_enabled` guard at each conditional order creation entry point, logging `SKIPPED — channel execute is OFF` when blocked.
