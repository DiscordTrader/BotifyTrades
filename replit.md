...
- All settings stored in SQLite `channels` table with columns: `profit_target_4_pct`, `profit_target_qty_1-4`, `trim_order_mode`, `trim_limit_offset`, `exit_strategy_mode`

**Exit Strategy Mode (Per-Channel)**: Each channel can configure how exits are handled via `exit_strategy_mode`:
- `signal`: Follow the trader's exit signals (trims/stops) - default for channels like Bishop
- `risk`: Use automated risk management exits (profit targets, stop loss, trailing stops)
- `hybrid`: Use both - trader signals trigger immediate exits, risk management provides backup protection

**Position Matching for Ambiguous Exit Signals**: When exit signals (like "stopped out") don't include contract details, the system automatically matches to the most recent open position from that channel using `get_most_recent_open_lot()`. This enables tracking of trader exits even when they only post the exit price.
