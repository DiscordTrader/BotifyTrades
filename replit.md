...
Core technologies include `discord.py-self` and `webull`. It employs a true dual-broker architecture for live and paper trading, with platform-specific credential encryption. Order execution uses an asynchronous, queue-based system. Signal parsing uses a multi-layer approach supporting learned formats, built-in regex, and AI fallback. **Signal Format Registry** (Jan 2026) provides industry-standard modular signal parsing with priority-based format matching. Supported trader formats include: **STACK$** (combined strike+type like 185p/6925c, optional expiry for 0DTE, ticker-less exits), **Jake/Optioneering** (+/- qty notation like +5 IWM @ lim0.08, full option format, order executed notifications), and **Slem** ($TSLA 445p 4.18 1/9 exp format). Risk management includes automated profit targets, stop losses, trailing stops, intelligent price slippage protection, and auto-quantity calculation, all GUI-configurable and stored in SQLite. Pre-trade analysis uses technical indicators, and post-execution analysis leverages OpenAI GPT models. Real-time market data is integrated, and interactive Discord commands enable on-demand analysis. The Auto Signal Conversion system executes stock alerts as Alpaca BRACKET ORDERs. An error monitoring system provides automatic detection, logging, and AI assistant contextual help.

The **Signal Verification Service** detects paper trading and impossible fills using real-time broker data, historical quote capture, time-window tolerance, and confidence scoring. Key features include entry-time verification, order size analysis, red flag detection (e.g., position size exceeding liquidity, impossible fills, extreme price deviation), and a tiered confidence scoring system. Now supports 6 real-time data sources: Webull, Tastytrade, Schwab, IBKR, Robinhood, and yfinance fallback. All async broker integrations use thread-safe bridge patterns (`_run_schwab_async_safe`, `_run_ibkr_async_safe`, `_run_robinhood_async_safe`) that execute in dedicated event loops to avoid deadlocks from Discord/Flask contexts. **IBKR QA Integration** (Jan 2026) provides TWS/Gateway-based trading with full BrokerSyncService position/order syncing and real-time quote retrieval (`get_quote_detailed`, `get_option_quote`). **Tastytrade QA Integration** (Jan 2026) provides OAuth2-authenticated trading with 15-minute session refresh for both LIVE and PAPER modes. **Robinhood QA Integration** (Jan 2026) provides LIVE-ONLY trading (no paper mode) with TOTP 2FA support and prominent warnings throughout the UI.

The system supports a dual-mode channel system for simultaneous execution and signal forwarding, FIFO-based P&L tracking, and Multi-Broker Execution across multiple accounts with per-channel broker selection. It handles market orders, comprehensive PNL page filtering, and per-channel position sizing. Per-channel risk settings allow independent operation, supporting 4-tier profit targets with customizable trim quantities, Market/Limit trim order modes, trailing stops, and **Leave Runner functionality**. **Per-Channel Slippage Protection** (Jan 2026) allows each channel to override global slippage settings with channel-specific thresholds via `get_effective_slippage_settings(channel_id)` function that prioritizes channel settings with global fallback. **Exit Strategy Mode** allows configuration per channel to follow trader signals, automated risk management, or both. **Position Matching for Ambiguous Exit Signals** automatically links exit signals to the most recent open position. The Trade Monitor feature automatically detects and posts broker-executed trades as BTO/STC signals to Discord.
...
**Dual-Action Channel Mappings** support simultaneous execution and signal forwarding via `execute_on_source` and `forward_enabled` flags, with flexible destination types. **Telegram Integration** supports reading trading signals from Telegram groups/channels. **Market-Specific Channel Pages** provide dedicated management for India Markets (NSE/BSE/MCX with DhanQ, Upstox, Zerodha), and Canada Markets (TSX/CSE/NEO with Questrade). **India Markets Page Redesign** (Jan 2026) provides a unified broker selector with consistent quantum theme styling (mint/violet/blue gradient), compact stat cards, tabbed data views (Positions, Orders, Trades, Conditional, AMO), and unified API endpoints (`/api/india/<broker>/*`) supporting all three India brokers equally. Market isolation ensures India orders, P&L, and risk management are completely separate from US market workflows.
...
**Filled Orders Tracking** syncs filled orders from broker APIs (Webull, Alpaca, Robinhood, Zerodha) into a local database table, featuring automatic sync, deduplication, multi-broker support, and a dedicated UI tab. **India Broker Sync** (Jan 2026) extends BrokerSyncService to sync positions and orders from all connected India brokers using async-safe patterns with `asyncio.run_coroutine_threadsafe`. **Execution-Based P&L Tracking** provides professional-grade P&L calculation based on actual broker fills rather than theoretical signal prices, including slippage tracking, latency metrics, and race condition protection. The **Pending Order Metadata Bridge** (Jan 2026) captures signal context at order placement (channel_id, message_id, timestamps, analyst sizing, PositionSizingService outputs) and hydrates execution_lots when fills arrive via BrokerSyncService. Uses BEGIN IMMEDIATE atomic transactions for FIFO integrity in concurrent STC fills.
...
The architecture is modular, structured into `src/` and `gui_app/` directories. Configuration uses database-stored encrypted credentials, with `config.ini` as a fallback. It features robust error handling, logging, and a multi-broker abstraction for Webull, Alpaca, Interactive Brokers, Tastytrade, Robinhood, Charles Schwab, Questrade, Upstox, Zerodha, and DhanQ. **Upstox Integration** provides V3 HFT API trading for Indian markets with auto-slicing and a pending order queue. **Charles Schwab Integration** (Jan 2026 QA complete) provides OAuth2-authenticated trading with automatic token refresh. The SchwabTokenManager singleton handles token lifecycle: access tokens (30-minute expiry) are auto-refreshed 5 minutes before expiry, while refresh tokens last 7 days requiring weekly re-authorization. **Automatic OAuth Callback** (Jan 2026) implements industry-standard OAuth flow with: temporary HTTPS callback server on port 8182 using self-signed certificates, PKCE (Proof Key for Code Exchange) for enhanced security, automatic code capture and token exchange, secure token storage using OS keyring (macOS Keychain, Windows Credential Manager) with encrypted file fallback, and multi-account support with user profiles in SQLite. Key files: `gui_app/schwab_oauth_server.py` (callback server + PKCE), `gui_app/schwab_token_storage.py` (secure storage). Register callback URL in Schwab portal as: `https://127.0.0.1:8182/callback`. **Full US Broker QA Pipeline** (Jan 2026 complete) provides consistent integration patterns across all supported US brokers (Schwab, IBKR, Tastytrade, Robinhood): BrokerSyncService position/order syncing with async-to-thread adapters, SignalVerificationService real-time stock and option quotes (`get_quote_detailed`, `get_option_quote`), RiskManager position monitoring with broker-specific fetch methods (`_fetch_schwab_positions`, `_fetch_ibkr_positions`, `_fetch_tastytrade_positions`, `_fetch_robinhood_positions`), conditional order routing support, and System Health status checks. UI templates use consistent broker identifiers: `SCHWAB`, `IBKR`, `IBKR_PAPER`, `TASTYTRADE_LIVE`, `TASTYTRADE_PAPER`, `ROBINHOOD` for channel configuration.
