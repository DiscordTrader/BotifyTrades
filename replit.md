...
- **Risk Engine Staleness Gate**: Implements pre-execution staleness protection for risk exits with REST-validated override. When REST API confirms a price (same or different), the staleness block is bypassed. States: `_rest_confirmed_this_cycle` (REST returned different price, per-cycle), `_rest_validated_same` (REST confirmed same price is real, 30s TTL). Repair cycles allow exit evaluation only when REST-confirmed. All state dicts are cleared on empty positions and stale entries cleaned on position removal/price change. `dynamic_sl` is classified as SL-type for market order routing.
