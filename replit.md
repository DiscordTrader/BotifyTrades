...
- **Fast Exit Tuning (Phase 1)**: Exit retry backoff reduced from 15s/30s/60s/120s to 3s/6s/10s/10s. Emergency backoff reduced from 5s/10s/15s to 3s/6s/9s/10s. Market order threshold lowered from 3 to 2 failed limit orders. Penny stocks (<$1) always use bid-price exits and force market orders. Extended-hours market-to-limit conversion uses 8% offset for penny stocks (vs 3% for normal). `find_open_bto_trade()` includes PENDING status for faster exit attribution. Backup thread reduced from 10s to 5s.
...
- **IBKRDataHub Streaming**: Provides real-time IBKR position and price streaming via `ib_insync` native events, managing `reqMktData` subscriptions and integrating with the risk engine. `get_quote()` enforces `QUOTE_STALE_THRESHOLD` (30s) by default — stale quotes are rejected unless a larger `max_age` is explicitly passed.
...
- **T212 Alt-Broker REST Quote Fallback**: When T212 is the broker for a conditional order and the symbol isn't in the T212 portfolio (no hub price available), both `StreamingPriceMonitor` and `BrokerPriceMonitor` now fall through to other connected brokers' REST `get_quote()` APIs (Webull → Schwab → Alpaca etc.). Uses live reference to `broker_instances` dict so brokers added after monitor creation are automatically visible.
...
- **Conditional Order Streaming Architecture**: Uses `StreamingPriceMonitor` and `BrokerPriceMonitor` with a robust fallback chain for price data.
