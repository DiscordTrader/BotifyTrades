...
- **EMA-5 Candlestick Risk Engine** (`src/risk/ema_engine.py`): Builds OHLC candles from WebSocket streaming ticks and computes rolling EMA for exit/escalation signals. Priority 2.5 in risk chain (after Dynamic SL, before Giveback Guard). Components: CandleAggregator (per-symbol thread-safe candle builder), EMAEngine (SMA-seeded rolling EMA), EMAExitEvaluator (pure function cross detection for CALL/PUT/STOCK), CandlePreWarmService (singleton managing all symbol tracking, pre-warms SPY/QQQ/SPX/NDX at startup, dynamic subscription for other symbols up to 50). Respects exit_strategy_mode gating (signal=disabled, risk=active, hybrid=arbiter). Per-channel settings in channels.js UI with cyan/teal theme.
