...
- **Risk Engine Direct Exit Architecture**: Dual-path exit execution system (primary queue-based, backup daemon thread for direct calls) with an Exit Lease Manager to prevent duplicate exits.
...
- **Trading212 Data Hub & Integration**: Provides a portfolio-based price cache for T212 positions, integrating with the risk engine and conditional order service. Features a real-time T212 architecture with cross-broker streaming prices, scheduled snapshots, SSE push to frontend, and hub-first risk engine.
...
- **Cross-Broker Price Recovery**: When a broker's streaming price is stuck, the system now: (1) subscribes ALL stock symbols to alternative streaming hubs (not just non-streaming brokers), (2) runs subscriptions BEFORE the stuck-price detection pass, (3) uses session-aware freshness windows for cross-hub lookups (5s regular / 15s extended vs previous 2s), (4) enables cross-broker streaming price overlay for all brokers (Webull→Schwab and Schwab→Webull), and (5) prioritizes the alternative broker's REST API before the native broker's REST when streaming is stuck. Logs now show specific REST source (e.g., `REST/Schwab`).
