...
**Universal Order Placement Resilience Layer**: 3-layer architecture (`src/services/order_resilience.py`) providing fault tolerance across all 11 brokers. Layer 1: BrokerErrorClassifier standardizes broker-specific errors into 5 categories (TRANSIENT_BUSY, RATE_LIMIT, PENDING_CONFLICT, TOKEN_EXPIRED, FATAL). Layer 2: OrderCircuitBreaker (per broker+symbol, thread-safe) prevents cascading failures with broker-specific thresholds (e.g., Webull 3/30s, IBKR 5/10s, Robinhood 2/60s). Layer 3: OrderResilienceLayer orchestrates unified retry budgets (budget=1 for risk orders eliminating the 5×3→5×1 retry storm, budget=3 for normal orders). Zero-latency success path (single dict lookup pre-check). Integrated at the `execute_on_single_broker` universal entry point. Webull brokers honor `_skip_internal_retry` flag to prevent nested retry layers.

**Live Monitoring Performance Layer**: Industry-grade position monitoring with zero-lag UX. Background snapshot daemon (`gui_app/live_snapshot.py`) fetches positions from all brokers in parallel every 5 seconds using ThreadPoolExecutor(6 workers), stores in thread-safe memory cache. `/api/trades/live-snapshot` returns cached data in <50ms. Frontend uses single unified 4-second poll (replacing 3 competing intervals), in-place DOM updates (no table destruction), Bid/Mid/Market quick-close buttons (no confirmation dialogs), toast notifications, price flash highlighting, staleness indicator, and emergency Close All button. Supports Schwab/IBKR/Tastytrade/Robinhood/Alpaca/Webull close routing through unified endpoint. **Dashboard Risk Engine Indicators**: Each position card on Dashboard → Live Monitoring includes a Risk Engine status strip showing ACTIVE/Not Monitored badges with live risk state details (SL, PT, DSL, Trailing, Early Trail, Giveback, Peak P&L). Hover tooltips show entry/current price, P&L, stop levels, and targets. Position matching uses trade_id (primary) and computed position_key (fallback) for reliable binding including manual positions. Risk states refresh with each 30-second data poll cycle.
