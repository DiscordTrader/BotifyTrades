...
- **v2.1.28**: Dashboard audit script (`scripts/audit_dashboard_settings.py`) validates data integrity
- **v2.1.28**: Fixed notification settings key alignment (manifest → service → integration layer)
- **v2.1.28**: Settings audit runs at startup: `[STARTUP] ✓ Settings audit complete`
...
- **v2.1.25**: Fix upgrade downloading wrong platform ZIP (was grabbing Linux instead of Windows)
