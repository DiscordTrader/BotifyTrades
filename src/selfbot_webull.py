...
# Import Risk Management module (pluggable architecture)
try:
...
class WebullBroker:
  ...
  did: sessionStorage.did || localStorage.did || sessionStorage.deviceId || localStorage.deviceId || ''
    ...
    async def get_account_info(self) -> dict:
        ...
        def _blocking_get_account():
            ...
            try:
                ...
                print(f"[Webull] [DEBUG] Final: BP=${buying_power:.2f}, OptBP=${options_bp:.2f}, PortValue=${portfolio_value:.2f}")
                return result
            except Exception as e:
    ...
    async def monitor_positions(self, order_queue: asyncio.Queue, alpaca_broker=None):
        """Background task to monitor positions and manage risk
        
        RISK MANAGEMENT HIERARCHY:
        ==========================
        1. Per-channel settings (HIGHEST PRIORITY) - Applied to trades with channel_id
        2. Global settings (FALLBACK) - Applied to trades WITHOUT channel_id when global is enabled
        
        MODES:
        - Global ENABLED: Per-channel first, then global fallback for unassigned trades
        - Global DISABLED: Only per-channel applies (trades without channel_id skip risk)
        
        Args:
            order_queue: Queue for exit orders
            alpaca_broker: Optional AlpacaBroker instance for monitoring Alpaca positions
        """
        # Fetch latest risk management settings from database
        _risk_settings = get_risk_management_settings()
        global_enabled = _risk_settings['enabled']
        
        # Count channels with per-channel risk settings
        channel_count = 0
        if DATABASE_MODULE_AVAILABLE:
            try:
                from gui_app import database as db
                ...
                print(f"[RISK] Warning: Could not check channel settings: {e}")
        
        # Display clear mode summary
        print(f"[RISK] ========== RISK MANAGEMENT STATUS ==========")
        print(f"[RISK] Global Risk: {'✓ ENABLED' if global_enabled else '✗ DISABLED'}")
        if global_enabled:
            print(f"[RISK]   → Profit Target: {_risk_settings['profit_target_percent']}%")
            print(f"[RISK]   → Stop Loss: {_risk_settings['stop_loss_percent']}%")
            print(f"[RISK]   → Trailing Stop: {_risk_settings['trailing_stop_percent']}%")
        print(f"[RISK] Per-Channel Risk: {channel_count} channel(s) configured")
        print(f"[RISK] ===============================================")
        
        if global_enabled and channel_count > 0:
            print(f"[RISK] MODE: HYBRID - Per-channel first, Global fallback")
            print(f"[RISK]   → Trades with channel_id: Use per-channel settings")
            print(f"[RISK]   → Trades without channel_id: Use global settings")
        elif global_enabled:
        ...
        # Load per-position stop/target prices from database
        if DATABASE_MODULE_AVAILABLE:
            try:
                from gui_app import database as db
                conn = db.get_connection()
                ...
                for row in rows:
                    ...
                    # Create position key (same as monitoring loop)
                    if asset_type == 'option':
                        strike = row[4]
                        expiry = row[5]
                        call_put = row[6]
                        pos_key = f"{symbol}_{strike}_{expiry}_{call_put}"
                    else:
                        pos_key = f"{symbol}_stock"
                    
                    # Update cache with database values
                    if pos_key in position_cache:
                        position_cache[pos_key]['stop_loss_price'] = sl_price
                        position_cache[pos_key]['profit_target_price'] = target_price
                        print(f"[RISK] Loaded from DB: {pos_key} SL=${sl_price} Target=${target_price}")
                
                print(f"[RISK] ✓ Loaded stop/target prices for {len(rows)} positions from database")
            except Exception as e:
                print(f"[RISK] Warning: Could not load stop/target prices from database: {e}")
        
        # Helper function to get per-channel risk settings
        def get_channel_risk_settings(symbol: str, asset_type: str, strike=None, expiry=None, call_put=None, broker_name=None) -> dict:
            """Get per-channel risk settings for a position.
            
            IMPORTANT: Risk settings are ONLY applied when:
            1. The trade has a valid channel_id linked to a channel in the database
            2. That channel has risk settings configured (profit targets, stop loss, or trailing stop)
            
            Positions without channel associations (e.g., manually entered trades, synced positions)
            will NOT have risk settings applied unless global risk management is enabled.
            """
            ...
            try:
                ...
                # Find the trade's channel_id and join with channel settings
                # Risk settings ONLY apply if trade has a valid channel_id linked to a channel
                if asset_type == 'option':
                    cursor.execute('''
                ...
                # Only return settings if:
                # 1. Trade exists with a valid channel_id (row[0] is not None)
                # 2. Channel has risk settings configured (row[1] is not None)
                if row and row[0] is not None and row[1] is not None:
                    return {
                        'channel_id': row[0],
                        'channel_name': row[7],
                        'profit_target_1_pct': row[1],
                        'profit_target_2_pct': row[2],
                        'profit_target_3_pct': row[3],
                        'stop_loss_pct': row[4],
                        'trailing_stop_pct': row[5],
                        'trailing_activation_pct': row[6]
                    }
                
                # No channel association or no risk settings - position will not be risk-managed
                # (unless global risk management is enabled)
                return None
            except Exception as e:
                print(f"[RISK] Warning: Could not fetch per-channel settings: {e}")
                return None
        
        while True:
            try:
                # Fetch latest risk management settings at each iteration
                _risk_settings = get_risk_management_settings()
                global_risk_enabled = _risk_settings['enabled']
                
                # Get global defaults (may be 0 if disabled)
                profit_target_pct = _risk_settings['profit_target_percent']
                stop_loss_pct = _risk_settings['stop_loss_percent']
                trailing_stop_pct = _risk_settings['trailing_stop_percent']
                
                # Check if we should continue monitoring
                # We continue if: (1) global risk is enabled, OR (2) any channel has risk settings
                if not global_risk_enabled:
                    # Check if any channel has risk settings configured
                    has_channel_risk = False
                    if DATABASE_MODULE_AVAILABLE:
                        try:
                            conn = db.get_connection()
                            cursor = conn.cursor()
                            ...
                            has_channel_risk = count > 0
                        except Exception as e:
                            print(f"[RISK] Warning: Could not check channel risk settings: {e}")
                    
                    if not has_channel_risk:
                        print("[RISK] Risk management disabled globally and no channel settings - stopping monitoring")
                        return
                    else:
                        # Continue but log that we're using channel-level settings only
                        pass  # Don't log every cycle, just continue
                
                await asyncio.sleep(MONITORING_INTERVAL)
                
                # Get current open positions from ALL brokers
                positions = []
                
                # 1. Get Webull positions
                webull_positions = await self.get_positions() or []
                if webull_positions:
                    print(f"[RISK] Webull returned {len(webull_positions)} positions:")
                    for pos in webull_positions:
                        pos['broker'] = 'Webull'  # Tag with broker
                        print(f"[RISK]   Webull position: {pos.get('symbol')} | asset={pos.get('asset')} | qty={pos.get('quantity')} | strike={pos.get('strike')} | expiry={pos.get('expiry')}")
                else:
                    print("[RISK] Webull returned 0 positions")
                positions.extend(webull_positions)
                
                # 2. Get Alpaca positions (if broker available)
                if alpaca_broker and alpaca_broker.connected:
                    try:
                        # Get raw Alpaca positions
                        alpaca_raw = await asyncio.to_thread(
                            alpaca_broker.trading_client.get_all_positions
                        )
                        
                        for ap in alpaca_raw:
                            # Check if it's an option (symbol contains space or option-like format)
                            symbol = ap.symbol
                            # Options have format: ALAB251205C00180000 (no spaces) or AAPL  250117C00230000 (with spaces)
                            is_option = '  ' in symbol or len(symbol) > 10
                            
                            # Debug: Log each position being processed (uses [RISK] tag so it shows in console)
                            print(f"[RISK] Alpaca position: {symbol} | is_option={is_option} | avg_cost={ap.avg_entry_price} | qty={ap.qty}")
                            
                            if is_option:
                                # Parse option symbol in OCC format
                                try:
                                    # Handle both formats: with spaces or without
                                    if '  ' in symbol:
                                        # Format with spaces: AAPL  250117C00230000
                                        parts = symbol.split()
                                        underlying = parts[0]
                                        option_code = parts[-1]
                                    else:
                                        # Format without spaces: ALAB251205C00180000
                                        ...
                                        if match:
                                            ...
                                            call_put_char = match.group(3)
                                            strike_raw = match.group(4)
                                            
                                            # Parse expiry: YYMMDD
                                            exp_yy = date_part[:2]
                                            exp_mm = date_part[2:4]
                                            exp_dd = date_part[4:6]
                                            ...
                                            positions.append({
                                                ...
                                                'strike': strike,
                                                'expiry': expiry,
                                                'direction': call_put_char,  # Keep as C/P to match database format
                                                'broker': 'ALPACA_PAPER',
                                                'raw_symbol': symbol
                                            })
                                            continue
                                        else:
                                            print(f"[RISK] Warning: Could not parse Alpaca option symbol (no regex match): {symbol}")
                                            continue
                                    
                                    # Parse for spaced format
                                    option_code = parts[-1]  # e.g., 250117C00230000
                                    ...
                                    call_put = option_code[6]  # C or P
                                    strike = float(option_code[7:]) / 1000  # Strike in thousandths
                                    ...
                                    positions.append({
                                        ...
                                        'strike': strike,
                                        'expiry': expiry,
                                        'direction': call_put,  # Keep as C/P to match database format
                                        'broker': 'ALPACA_PAPER',
                                        'raw_symbol': symbol  # Keep original for order execution
                                    })
                                except Exception as parse_err:
                                    print(f"[RISK] Warning: Could not parse Alpaca option symbol {symbol}: {parse_err}")
                            else:
                                # Stock position
                                positions.append({
                                    'symbol': symbol,
                        ...
                        if alpaca_raw:
                            print(f"[RISK] Found {len(alpaca_raw)} Alpaca positions")
                    except Exception as e:
                        print(f"[RISK] Warning: Could not fetch Alpaca positions: {e}")
                
                # Track which positions we've seen from broker
                broker_position_keys = set()
                
                if positions:
                    webull_count = sum(1 for p in positions if p.get('broker') == 'Webull')
                    alpaca_count = sum(1 for p in positions if p.get('broker') == 'ALPACA_PAPER')
                    print(f"\n[RISK] Monitoring {len(positions)} open positions (Webull: {webull_count}, Alpaca: {alpaca_count})...")
                
                for pos in (positions or []):
                    try:
                        # Create unique key for this position (include broker to distinguish Webull vs Alpaca)
                        broker_tag = pos.get('broker', 'Webull')
                        if pos['asset'] == 'option':
                            pos_key = f"{broker_tag}_{pos['symbol']}_{pos.get('strike', '')}_{pos.get('expiry', '')}_{pos.get('direction', '')}"
                        else:
                            pos_key = f"{broker_tag}_{pos['symbol']}_stock"
                        
                        # Debug: Show which positions are being processed
                        print(f"[RISK] Processing position: {pos_key} | entry=${pos.get('avg_cost', 0):.2f} | current=${pos.get('current_price', 0):.2f}")
                        
                        # Track that we've seen this position from broker
                        broker_position_keys.add(pos_key)
                    
                        # Initialize cache entry if new position
                        if pos_key not in position_cache:
                            # Fetch stop/target prices from database for this position
                            sl_price = None
                            target_price = None
                            ...
                            if DATABASE_MODULE_AVAILABLE:
                                try:
                                    ...
                                    if pos['asset'] == 'option':
                                        cursor.execute('''
                                            SELECT stop_loss_price, profit_target_price
                                            FROM trades
                                            WHERE symbol = ? AND asset_type = 'option' AND strike = ? AND expiry = ? AND call_put = ?
                                            AND status = 'OPEN' AND direction = 'BTO'
                                            ORDER BY id DESC LIMIT 1
                                        ''', (pos['symbol'], pos.get('strike'), pos.get('expiry'), pos.get('direction')))
                                    else:
                                        cursor.execute('''
                                            SELECT stop_loss_price, profit_target_price
                                            FROM trades
                                    ...
                                    print(f"[RISK] Warning: Could not fetch stop/target from DB for {pos_key}: {e}")
                            ...
                            position_cache[pos_key] = {
                                ...
                                'broker': broker_tag,  # Track which broker owns this position
                                'raw_symbol': pos.get('raw_symbol')  # For Alpaca option symbols
                            }
                            
                            if sl_price or target_price:
                                print(f"[RISK] New position tracked: {pos_key} @ ${pos['avg_cost']:.2f} | SL: ${sl_price or 'N/A'} | Target: ${target_price or 'N/A'}")
                            else:
                                print(f"[RISK] New position tracked: {pos_key} @ ${pos['avg_cost']:.2f}")
                        
                        cache = position_cache[pos_key]
                        
                        # Skip if already closing (with auto-reset after 3 cycles to prevent stuck positions)
                        if cache.get('closing', False):
                            # Track how many cycles this position has been "closing"
                            closing_cycles = cache.get('closing_cycles', 0) + 1
                            cache['closing_cycles'] = closing_cycles
                            
                            # Auto-reset after 3 cycles (~90 seconds) if position still exists
                            if closing_cycles >= 3:
                                print(f"[RISK] ⚠️ {pos_key}: Position still exists after {closing_cycles} closing cycles - resetting closing flag")
                                cache['closing'] = False
                                cache['closing_cycles'] = 0
                            else:
                                continue
                        ...
                        # Calculate profit/loss percentage (with division by zero protection)
                        entry = cache['entry_price']
                        current = pos['current_price']
                        if entry <= 0:
                            # Can't calculate P&L without valid entry price - use 0%
                            pct_change = 0.0
                            print(f"[RISK] ⚠️ {pos_key}: Entry price is {entry}, cannot calculate P&L")
                        else:
                            pct_change = ((current - entry) / entry) * 100
                        
                        # Get per-channel risk settings (priority over global)
                        channel_settings = cache.get('channel_settings')
                        if channel_settings is None:
                            # Get broker name for fallback lookup
                            pos_broker = pos.get('broker', 'Webull')
                            # Normalize broker name (e.g., "Alpaca Paper" -> "ALPACA_PAPER")
                            if 'alpaca' in pos_broker.lower():
                                if 'paper' in pos_broker.lower():
                                    pos_broker = 'ALPACA_PAPER'
                                else:
                                    pos_broker = 'ALPACA_LIVE'
                            
                            if pos['asset'] == 'option':
                                # Convert 'CALL'/'PUT' to 'C'/'P' for database lookup
                                ...
                                if direction == 'CALL':
                                    ...
                                    call_put = direction[0] if direction else None
                                
                                channel_settings = get_channel_risk_settings(
                                    pos['symbol'], 'option',
                                    pos.get('strike'), pos.get('expiry'), call_put,
                                    broker_name=pos_broker
                                )
                            else:
                                channel_settings = get_channel_risk_settings(pos['symbol'], 'stock', broker_name=pos_broker)
                            cache['channel_settings'] = channel_settings  # Cache it
                            if channel_settings:
                                print(f"[RISK] Using per-channel settings from '{channel_settings.get('channel_name', 'Unknown')}': "
                                      f"Targets={channel_settings.get('profit_target_1_pct')}%/{channel_settings.get('profit_target_2_pct')}%/{channel_settings.get('profit_target_3_pct')}%, "
                                      f"StopLoss={channel_settings.get('stop_loss_pct')}%")
                        
                        # Determine which risk settings to use (per-channel > global)
                        if channel_settings:
                            pos_profit_target_1 = channel_settings.get('profit_target_1_pct') or 0
                            pos_profit_target_2 = channel_settings.get('profit_target_2_pct') or 0
                            pos_profit_target_3 = channel_settings.get('profit_target_3_pct') or 0
                            pos_stop_loss_pct = channel_settings.get('stop_loss_pct') or stop_loss_pct
                            pos_trailing_stop_pct = channel_settings.get('trailing_stop_pct') or trailing_stop_pct
                            pos_trailing_activation = channel_settings.get('trailing_activation_pct') or TRAILING_ACTIVATION_PCT
                        else:
                            # If global risk is disabled and no channel settings, skip this position
                            if not global_risk_enabled:
                                continue  # Skip position - no risk settings apply
                            
                            pos_profit_target_1 = profit_target_pct
                            pos_profit_target_2 = 0
                            pos_profit_target_3 = 0
                            pos_stop_loss_pct = stop_loss_pct
                            pos_trailing_stop_pct = trailing_stop_pct
                            pos_trailing_activation = TRAILING_ACTIVATION_PCT
                        
                        # Show stop/target prices if set
                        sl_price = cache.get('stop_loss_price')
                        target_price = cache.get('profit_target_price')
                        
                        # Build display string
                        channel_name = channel_settings.get('channel_name', 'Global') if channel_settings else 'Global'
                        if sl_price or target_price:
                            print(f"[RISK] [{channel_name}] {pos_key}: ${current:.2f} ({pct_change:+.2f}%) | Entry: ${entry:.2f} | SL: ${sl_price or 'N/A'} | Target: ${target_price or 'N/A'} | Qty: {pos['quantity']}")
                        elif channel_settings:
                            print(f"[RISK] [{channel_name}] {pos_key}: ${current:.2f} ({pct_change:+.2f}%) | Entry: ${entry:.2f} | Targets: {pos_profit_target_1}/{pos_profit_target_2}/{pos_profit_target_3}% | SL: {pos_stop_loss_pct}% | Qty: {pos['quantity']}")
                        else:
                            print(f"[RISK] [Global] {pos_key}: ${current:.2f} ({pct_change:+.2f}%) | Entry: ${entry:.2f} | Qty: {pos['quantity']}")
                        
                        # Check exit conditions
                        should_exit = False
                        exit_reason = ""
                        exit_qty = int(pos['quantity'])  # Default to full position
                        
                        # PRIORITY 1: Per-signal price-based stops (if set)
                        if target_price and current >= target_price:
                            should_exit = True
                            exit_reason = f"PROFIT TARGET PRICE (${current:.2f} >= ${target_price})"
                        elif sl_price and current <= sl_price:
                            should_exit = True
                            exit_reason = f"STOP LOSS PRICE (${current:.2f} <= ${sl_price})"
                        
                        # PRIORITY 2: Per-channel tiered profit targets (scale out)
                        # For small positions (1-2 contracts), just close all at first target hit
                        # For larger positions, scale out: 1/3 at tier 1, 1/2 remaining at tier 2, all at tier 3
                        elif channel_settings and not cache.get('tier1_hit') and pos_profit_target_1 > 0 and pct_change >= pos_profit_target_1:
                            should_exit = True
                            cache['tier1_hit'] = True
                            current_qty = int(pos['quantity'])
                            
                            # For small positions, close all at first target
                            if current_qty <= 2:
                                exit_qty = current_qty
                                exit_reason = f"TIER 1 TARGET [{channel_name}] ({pct_change:.2f}% >= {pos_profit_target_1}%) - Closing all {exit_qty} (small position)"
                                cache['tier2_hit'] = True  # Skip tier 2
                                cache['tier3_hit'] = True  # Skip tier 3
                            else:
                                exit_qty = max(1, current_qty // 3)  # Sell 1/3 at tier 1
                                exit_reason = f"TIER 1 TARGET [{channel_name}] ({pct_change:.2f}% >= {pos_profit_target_1}%) - Selling {exit_qty} of {current_qty}"
                        
                        elif channel_settings and cache.get('tier1_hit') and not cache.get('tier2_hit') and pos_profit_target_2 > 0 and pct_change >= pos_profit_target_2:
                            should_exit = True
                            cache['tier2_hit'] = True
                            current_qty = int(pos['quantity'])
                            
                            # If only 1 left, close all
                            if current_qty <= 1:
                                exit_qty = current_qty
                                exit_reason = f"TIER 2 TARGET [{channel_name}] ({pct_change:.2f}% >= {pos_profit_target_2}%) - Closing remaining {exit_qty}"
                                cache['tier3_hit'] = True  # Skip tier 3
                            else:
                                exit_qty = max(1, current_qty // 2)  # Sell half remaining at tier 2
                                exit_reason = f"TIER 2 TARGET [{channel_name}] ({pct_change:.2f}% >= {pos_profit_target_2}%) - Selling {exit_qty} of {current_qty}"
                        
                        elif channel_settings and cache.get('tier2_hit') and not cache.get('tier3_hit') and pos_profit_target_3 > 0 and pct_change >= pos_profit_target_3:
                            should_exit = True
                            cache['tier3_hit'] = True
                            exit_qty = int(pos['quantity'])  # Close all remaining
                            exit_reason = f"TIER 3 TARGET [{channel_name}] ({pct_change:.2f}% >= {pos_profit_target_3}%) - Closing remaining {exit_qty}"
                        
                        # PRIORITY 3: Global percentage-based stops (fallback)
                        elif not channel_settings and profit_target_pct > 0 and pct_change >= profit_target_pct:
                            should_exit = True
                            exit_reason = f"PROFIT TARGET % [Global] ({pct_change:.2f}% >= {profit_target_pct}%)"
                        
                        # Stop loss check (uses per-channel if available)
                        elif pos_trailing_stop_pct == 0 and pos_stop_loss_pct > 0 and pct_change <= -pos_stop_loss_pct:
                            should_exit = True
                            exit_reason = f"STOP LOSS % [{channel_name}] ({pct_change:.2f}% <= -{pos_stop_loss_pct}%)"
                        
                        # Trailing stop logic (uses per-channel if available)
                        elif pos_trailing_stop_pct > 0:
                            # Activate trailing stop if profit threshold reached
                            if not cache['trailing_activated'] and pct_change >= pos_trailing_activation:
                                cache['trailing_activated'] = True
                                print(f"[RISK] [{channel_name}] {pos_key}: Trailing stop ACTIVATED at {pct_change:.2f}% gain")
                            
                            # If trailing stop active, check if price dropped below trailing threshold
                            if cache['trailing_activated']:
                                trailing_stop_price = cache['highest_price'] * (1 - pos_trailing_stop_pct / 100)
                                if current <= trailing_stop_price:
                                    should_exit = True
                                    exit_reason = f"TRAILING STOP [{channel_name}] (${current:.2f} <= ${trailing_stop_price:.2f}, dropped {pos_trailing_stop_pct}% from high ${cache['highest_price']:.2f})"
                            
                            # Also check fixed stop loss before trailing activates
                            elif pos_stop_loss_pct > 0 and pct_change <= -pos_stop_loss_pct:
                                should_exit = True
                                exit_reason = f"STOP LOSS [{channel_name}] ({pct_change:.2f}% <= -{pos_stop_loss_pct}%)"
                        ...
                        if should_exit:
                            ...
                            # Stop loss and trailing stop exits are ALWAYS full closes
                            # Only tiered profit targets (TIER 1/2 without TIER 3) are partial
                            is_stop_exit = 'STOP LOSS' in exit_reason or 'TRAILING STOP' in exit_reason
                            is_partial_close = (cache.get('tier1_hit') and not cache.get('tier3_hit')) and not is_stop_exit
                            
                            if not is_partial_close:
                                cache['closing'] = True
                                print(f"[RISK] Marked {pos_key} as closing (prevent duplicate triggers)")
                            
                            try:
                                # Create STC signal with exit_qty (may be partial for tiered targets)
                                stc_signal = {
                                    'asset': pos['asset'],
                                    'action': 'STC',
                                    'qty': exit_qty,  # Use calculated exit qty (full or partial)
                                    'symbol': pos['symbol'],
                                    'price': current,  # Market price for limit order
                                    'broker': broker_tag,  # CRITICAL: Route to correct broker!
                                    'raw_symbol': pos.get('raw_symbol'),  # For Alpaca option execution
                                    'exit_reason': exit_reason  # Track why position was closed
                                }
                                
                                # Look up the original BTO trade to get channel attribution
                                ...
                                if DATABASE_MODULE_AVAILABLE:
                                    try:
                                        ...
                                        if pos_call_put == 'CALL':
                                            pos_call_put = 'C'
                                        elif pos_call_put == 'PUT':
                                            pos_call_put = 'P'
                                        elif pos_call_put and len(pos_call_put) > 0:
                                            pos_call_put = pos_call_put[0]
                                        
                                        origin_trade = db.find_open_bto_trade(
                                            symbol=pos['symbol'],
                                            asset_type=pos['asset'],
                                            broker=broker_tag,
                                            strike=pos.get('strike'),
                                            expiry=pos.get('expiry'),
                                            call_put=pos_call_put
                                        )
                                        
                                        if origin_trade:
                                            stc_signal['channel_id'] = origin_trade['channel_id']
                                            stc_signal['message_id'] = origin_trade['message_id']
                                            stc_signal['origin_trade_id'] = origin_trade['id']
                                            
                                            # Get channel_record_id for database saving
                                            if origin_trade['channel_id']:
                                                try:
                                                    conn = db.get_connection()
                                                    cursor = conn.cursor()
                                                    cursor.execute('SELECT id FROM channels WHERE discord_channel_id = ?', 
                                                                 (str(origin_trade['channel_id']),))
                                                    channel_row = cursor.fetchone()
                                                    if channel_row:
                                                        stc_signal['channel_record_id'] = channel_row['id']
                                                except Exception as ce:
                                                    print(f"[RISK] Warning: Could not get channel_record_id: {ce}")
                                            
                                            # Determine risk_trigger type for tracking
                                            if 'STOP LOSS' in exit_reason:
                                                stc_signal['risk_trigger'] = 'stop_loss'
                                            elif 'TRAILING STOP' in exit_reason:
                                                stc_signal['risk_trigger'] = 'trailing_stop'
                                            elif 'TIER' in exit_reason or 'PROFIT' in exit_reason:
                                                stc_signal['risk_trigger'] = 'profit_target'
                                            else:
                                                stc_signal['risk_trigger'] = 'risk_management'
                                            
                                            print(f"[RISK] ✓ Linked to origin channel_id={origin_trade['channel_id']} (trade #{origin_trade['id']})")
                                        else:
                                            print(f"[RISK] ⚠️ No origin BTO trade found in database for {pos_key}")
                                            stc_signal['risk_trigger'] = 'risk_management'
                                    except Exception as e:
                                        print(f"[RISK] Warning: Could not lookup origin trade: {e}")
                                        stc_signal['risk_trigger'] = 'risk_management'
                                ...
                                # Mark as risk management order so worker knows to route correctly
                                stc_signal['_risk_management_order'] = True
                                await order_queue.put(stc_signal)
                                print(f"[RISK] STC order queued for {pos_key} via {broker_tag}: {stc_signal}")
                        ...
                        # Catch any exception during position processing so we don't skip other positions
                        print(f"[RISK] ⚠️ Error processing position {pos.get('symbol', 'UNKNOWN')}: {pos_err}")
                        import traceback
                        traceback.print_exc()
                ...
                # Sync database: auto-close positions that are OPEN in DB but not in broker account
                if DATABASE_MODULE_AVAILABLE:
                    try:
                        from gui_app import database as db
                        conn = db.get_connection()
                        ...
                        # Get all OPEN positions from database (include broker column)
                        cursor.execute('''
                            SELECT id, symbol, asset_type, strike, expiry, call_put, broker
                            FROM trades
                            WHERE status = 'OPEN' AND direction = 'BTO'
                        ''')
                        ...
                        for row in db_open_positions:
                            ...
                            db_symbol = row[1]
                            db_asset_type = row[2]
                            db_broker = row[6] or 'Webull'
                            
                            # Normalize broker name to match broker_position_keys format
                            if 'alpaca' in db_broker.lower():
                                db_broker_tag = 'ALPACA_PAPER'
                            else:
                                db_broker_tag = 'Webull'
                            
                            # Create position key for comparison (MUST include broker prefix to match broker_position_keys)
                            if db_asset_type == 'option':
                                db_strike = row[3]
                                db_expiry = row[4]
                                db_call_put = row[5]
                                # Keep C/P format as-is (matches broker position format)
                                # Format key: BROKER_SYMBOL_STRIKE_EXPIRY_CALLPUT (e.g., Webull_HIMS_40.0_12/05_C)
                                db_pos_key = f"{db_broker_tag}_{db_symbol}_{db_strike}_{db_expiry}_{db_call_put}"
                            else:
                                db_pos_key = f"{db_broker_tag}_{db_symbol}_stock"
                            
                            # If this position is NOT in the broker's position list, auto-close it
                            if db_pos_key not in broker_position_keys:
                                cursor.execute('''
                                    UPDATE trades
                                    SET status = 'CLOSED',
                                ...
                                ''', (db_id,))
                                conn.commit()
                                print(f"[RISK] ✓ Auto-closed orphaned position in database: {db_pos_key} (ID: {db_id})")
                    
                    except Exception as e:
                        print(f"[RISK] Warning: Could not sync database with broker positions: {e}")
...
class SelfClient(discord.Client):
    ...
    async def setup(self):
        ...
        # Use pluggable RiskManager module if available, otherwise fall back to legacy
        if RISK_MODULE_AVAILABLE:
            try:
                ...
                print("[RISK] ✓ RiskManager module initialized (pluggable architecture)")
            except Exception as e:
                print(f"[RISK] ⚠️ Failed to start RiskManager: {e}")
                import traceback
                traceback.print_exc()
        else:
            # Legacy fallback: use inline monitor_positions from WebullBroker
            has_channel_risk_settings = False
            if DATABASE_MODULE_AVAILABLE and not ENABLE_RISK_MGMT:
                try:
                    conn = db.get_connection()
                    cursor = conn.cursor()
                    cursor.execute('''
                    ...
                    ''')
                    count = cursor.fetchone()[0]
                    has_channel_risk_settings = count > 0
                    if has_channel_risk_settings:
                        print(f"[RISK] Found {count} channel(s) with per-channel risk settings")
                except Exception as e:
                    print(f"[RISK] Warning: Could not check channel risk settings: {e}")
            
            if ENABLE_RISK_MGMT or has_channel_risk_settings:
                try:
                    self.loop.create_task(self.broker.monitor_positions(self.order_queue, alpaca_broker=self.paper_broker))
                    print("[RISK] ✓ Legacy position monitoring started")
                except Exception as e:
                    print(f"[RISK] ⚠️ Failed to start position monitoring: {e}")
                    import traceback
                    traceback.print_exc()
            else:
                print("[Init] Position monitoring disabled (no global or per-channel risk settings)")
