  ...
  did: sessionStorage.did || localStorage.did || sessionStorage.deviceId || localStorage.deviceId || ''
    ...
    async def place_option_order(self, symbol: str = None, strike: float = None,
        ...
        def _blocking_place():
            ...
            if self._use_paper_account:
                sim_order_id = f"PAPER_SIM_{uuid.uuid4().hex[:16].upper()}"
                print(f"[{self.name}] 📋 Simulating paper option order (Webull paper API does not support options)")
                print(f"[{self.name}] ✓ Simulated {side} {int(adjusted_qty)} {symbol} option @ ${effective_price}")
                print(f"[{self.name}] ✓ Simulated order ID: {sim_order_id}")
                return {
                    ...
                    'data': {
                        ...
                        'status': 'Filled',
                        'filledPrice': float(effective_price),
                        'filledQuantity': int(adjusted_qty)
                    }
                }
...
Focus on: Why is this unusual? Bullish or bearish signal? Risk/reward assessment."""
                        ...
                        if market == 'US' and order.get('strike'):
                            ...
                            try:
                                ...
                                if qot_price and qot_price > 0:
                                    ...
                                    signal['is_market_order'] = False  # Force limit order when we have a price
    ...
    async def _process_message(self, message: discord.Message):
            ...
            if not message.content.strip().startswith('!'):
                ...
                if is_bto_stc_signal or is_bullwinkle or is_jacob or is_zscalps or is_jake or is_order_executed or is_bishop or is_evapanda or is_toon or is_spy_sniper_embed or is_sir_goldman:
                    ...
                    if should_forward and (is_webhook_dest or is_channel_dest):
                        ...
                        elif is_order_executed:
                            print(f"[DEBUG] Taking ORDER_EXECUTED path", flush=True)
                            oe_parsed = parse_order_executed_signal(combined_content)
                            if oe_parsed:
                                # Format as BTO/STC with qty
                                action = oe_parsed.get('action', 'BTO')
                                symbol = oe_parsed.get('symbol', '')
                                qty = oe_parsed.get('qty', 1)
                                strike = oe_parsed.get('strike', '')
                                opt_type = oe_parsed.get('opt_type', 'C')
                                expiry = oe_parsed.get('expiry', '')
                                price = oe_parsed.get('price')
                                price_str = f"@ {price}" if price else "@ m"
                                forward_msg = f"{action} {qty} {symbol} {strike}{opt_type} {expiry} {price_str}"
                                print(f"[CHANNEL MAP] ✓ Formatted Order Executed: {forward_msg}")
                            else:
                                forward_msg = combined_content.strip()
                    ...
                    try:
                        ...
                        if is_bullwinkle:
                            ...
                            oe_parsed = parse_order_executed_signal(combined_content)
                            if oe_parsed:
                                is_exit = oe_parsed.get('is_exit', False)
                                parsed_signal = {
                                    'symbol': oe_parsed.get('symbol', ''),
                                    'strike': oe_parsed.get('strike', 0),
                                    'opt_type': oe_parsed.get('opt_type', 'C'),
                                    'expiry': oe_parsed.get('expiry', ''),
                                    'price': oe_parsed.get('price', 0),
                                    'qty': oe_parsed.get('qty', 1),
                                    'is_exit': is_exit,
                                    'asset_type': 'option'
                                }
                            ...
                            if is_exit:
                                ...
                                if open_pos:
                                    ...
                                    if exit_result:
                                            ...
                                            if signal_inst_id:
                                                if fully_closed:
                                                    await signal_exit_manager.handle_exit_signal(
                                                        ...
                                                        broker=channel_broker,
                                                        symbol=symbol,
                                                        asset_type=parsed_signal.get('asset_type') or parsed_signal.get('asset', 'stock'),
                                        ...
                                        # Double-check: BUILD_TYPE must be 'ADMIN' (not just is_admin_build())
                                        trade_summary_enabled_for_channel = False
                                        if BUILD_TYPE != 'ADMIN' or not is_admin_build():
                                            print(f"[PNL TRACK] ⏭️ Trade Summary BLOCKED - USER build (BUILD_TYPE={BUILD_TYPE})")
                                        else:
                                            try:
                                        ...
                                        if BUILD_TYPE == 'ADMIN' and is_admin_build() and webhook_url and trade_summary_enabled_for_channel:
        ...
        if opt:
            ...
            # COMPLETELY DISABLED for USER builds - Trade Summary is admin-only feature
            # Double-check: BUILD_TYPE must be 'ADMIN' (not just is_admin_build() which could be overridden)
            if opt['action'] == 'STC' and opt.get('_pnl_result') and BUILD_TYPE == 'ADMIN' and is_admin_build():
            ...
            elif opt['action'] == 'STC' and opt.get('_pnl_result'):
                            ...
                            if converted:
                                ...
                                try:
                                    ...
                                    if broker_for_quote and hasattr(broker_for_quote, 'get_option_quote'):
                                        ...
                                        qqq_quote = await broker_for_quote.get_option_quote(
                                            symbol='QQQ',
